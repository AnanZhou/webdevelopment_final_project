...
My application is accessible at: http://127.0.0.1:5000/
Recommended Command to Start the App: flask run --reload --debug

Account Information:
For simplicity, you can use the following existing account and password. Alternatively, you can sign up with your own credentials:
Account: Unnamed User #307724
Password: ornmghjc5q

Features Overview:
1. Login:
After logging in, you'll land on the channel page.
The channel page includes options like Profile and Logout buttons.
Note: Avoid clicking the logout button immediately after logging in, as it may sometimes cause a server error.

2. Navigating Channels:
Click on any channel to navigate to the Message Page.
The message page displays:
A list of messages.
An input section to post new messages.

3. Emoji Reactions:
React to messages using a set of simple emojis.

4. Threaded Replies:
Click the Reply button to open the Replies Page.
On this page, you can:
Write and send replies.
Close the replies panel after replying.

5. Reply Count Update:
After adding a reply, the "Reply" button for the original message will update to show the total number of replies (e.g., 1 View Reply).

Important Notes:
Avoid frquent Refreshing the Page: Refreshing may cause unexpected errors.
If a refresh error occurs, return to the app by visiting:
http://127.0.0.1:5000/
